...
Esta funcionalidad ya se encuentra implementada en el proyecto. Resumen técnico:

- **`src/services/GithubService.ts`**: se agregaron las funciones `deleteRepository(owner, repoName)` (usa `apiClient.delete`) y `updateRepository(owner, repoName, payload)` (usa `apiClient.patch`), siguiendo el mismo patrón que `fetchRepositories` y `createRepository`.
- **`src/components/RepoItem.tsx`**: las opciones deslizables (`IonItemSliding`) de lápiz y basurero ahora están conectadas a la lógica real. El botón de basurero muestra una alerta de confirmación (`useIonAlert`) antes de ejecutar el `DELETE`. El botón de lápiz abre un modal de edición.
- **`src/components/EditRepoModal.tsx`** (nuevo): modal con formulario (nombre y descripción) que se precarga con los datos actuales del repositorio y ejecuta el `PATCH` al guardar.
- **`src/pages/Tab1.tsx`**: recibe callbacks `onDeleted` y `onUpdated` desde cada `RepoItem` para actualizar el estado (`repositoryList`) de forma dinámica, sin necesidad de volver a llamar a `GET /user/repos`.

Con esto queda cubierto el ciclo CRUD completo: **C**reate (Tab2 + POST), **R**ead (Tab1 + GET), **U**pdate (modal de edición + PATCH) y **D**elete (alerta de confirmación + DELETE).
...
3. Configurar el token de GitHub:
   ...
   - Agrega tu token: `VITE_GITHUB_API_TOKEN=tu_token_aquí`
   - El token debe tener el permiso (scope) `repo` para poder crear, editar y eliminar repositorios
